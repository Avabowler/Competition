...
def test_left_base_attacked_from_east():
    # 样例基地 (10,24) 在地图左半 -> 机器人从远侧（东）压过来
    turn = make_turn(10, 24)
    assert attack_side(turn) == "east"
    front = front_cells(turn)
    assert front and all(p.x == 13 for p in front)  # 东排 x = xmax+2 = 13


def test_left_base_east_row_built_first():
    turn = make_turn(10, 24)
    plan = wall_plan(turn)
    assert plan, "应有建造计划"
    assert all(p.x == 13 for p in plan[:4])         # 东排 4 格排最前


def test_left_base_door_on_west_back():
    door = entrance_pos(make_turn(10, 24))
    assert (door.x, door.y) == (8, 22)              # 背面（西）角格


def test_right_base_attacked_from_west():
    turn = make_turn(30, 10)
    assert attack_side(turn) == "west"
    front = front_cells(turn)
    assert front and all(p.x == 28 for p in front)  # 西排 x = xmin-2 = 28


def test_right_base_west_row_built_first_and_door_east():
    turn = make_turn(30, 10)
    plan = wall_plan(turn)
    assert plan
    assert all(p.x == 28 for p in plan[:4])         # 西排最前
    door = entrance_pos(turn)
    assert (door.x, door.y) == (33, 7)              # 背面（东）角格
